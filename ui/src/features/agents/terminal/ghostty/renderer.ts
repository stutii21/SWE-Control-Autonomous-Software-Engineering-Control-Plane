import { GHOSTTY_CELL_WIDE, ghosttyColorsEqual } from "./core"
import type { GhosttyCell, GhosttyColor, GhosttySnapshot } from "./core"
...
export function ghosttyTextRunEnd(
  cells: ReadonlyArray<GhosttyCell>,
  start: number,
...
export function renderGhosttySnapshot(options: {
  ...
  if (
    ...
    snapshot.cursorY >= 0 &&
    !rowsToDraw.includes(snapshot.cursorY)
  ) {
  ...
  for (const rowIndex of rowsToDraw) {
    ...
    while (backgroundStart < row.cells.length) {
      ...
      if (
        first.selected ||
        !ghosttyColorsEqual(first.background, snapshot.background)
      ) {
    ...
    while (runStart < row.cells.length) {
      ...
      const runEnd = ghosttyTextRunEnd(row.cells, runStart, (cell) =>
    ...
    for (let column = 0; column < row.cells.length; column += 1) {
      ...
      const hoveredLink =
        ...
        (rowIndex > hoveredLinkRange.start.y ||
          column >= hoveredLinkRange.start.x) &&
