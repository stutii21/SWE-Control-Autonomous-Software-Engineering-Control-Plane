...
import { GHOSTTY_CELL_WIDE } from "./core"
import {
  ghosttyTextRunEnd,
  measureGhosttyCell,
...
describe("terminalGridSize", () => {
  it("matches the mobile renderer's cell-and-padding sizing model", () => {
    expect(
      terminalGridSize(808, 408, { width: 10, height: 20, baseline: 15 }, 4)
    ).toEqual({
      cols: 80,
      rows: 20,
    })
  ...
  it("never sends an invalid zero-sized terminal to libghostty", () => {
    expect(
      terminalGridSize(0, 0, { width: 10, height: 20, baseline: 15 }, 4)
    ).toEqual({
      cols: 1,
      rows: 1,
    })
...
describe("renderGhosttySnapshot", () => {
  it("underlines every cell in a hovered wrapped link", () => {
    const fillRectCalls: Array<Array<number>> = []
    const context = {
      canvas: { width: 200, height: 80 },
      beginPath: () => {},
      clip: () => {},
      fillRect: (...args: Array<number>) => fillRectCalls.push(args),
      fillText: () => {},
      rect: () => {},
      resetTransform: () => {},
      restore: () => {},
  ...
  it("constrains text runs and cursor glyphs to their terminal cells", () => {
    const fillTextCalls: Array<Array<unknown>> = []
    const context = {
      canvas: { width: 200, height: 40 },
      beginPath: () => {},
      clip: () => {},
      fillRect: () => {},
      fillText: (...args: Array<unknown>) => fillTextCalls.push(args),
      rect: () => {},
      resetTransform: () => {},
      restore: () => {},
      save: () => {},
    ...
    const snapshot: GhosttySnapshot = {
      ...
      dirtyRows: new Set([0]),
      rowData: [
        { cells, text: "abx", isWrapContinuation: false, wrapsToNext: false },
      ],
    }
  ...
  it("repaints the previous cursor row after the cursor moves", () => {
    const clearedRows: Array<number> = []
    const context = {
      canvas: { width: 200, height: 80 },
      beginPath: () => {},
      clip: () => {},
